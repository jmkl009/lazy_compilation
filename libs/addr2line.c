...
slurp_symtab(bfd *abfd)
{
    ...
    void **syms_addr = (void **)&m_libtrace_data.syms;
...
find_address_in_section(bfd *abfd, asection *section, void *data)
{
    ...
    const char *filename = psi->filename;
    psi->found = bfd_find_nearest_line(abfd, section, 
                   m_libtrace_data.syms, psi->pc - vma,
                   &filename, &psi->functionname,
                   &psi->line);
    psi->filename = (char *)filename;
}
        ...
        if (buf_func != NULL)
          ...
          do {
            ...
            if (!m_libtrace_data.unwind_inlines)
                ...
                const char*filename = si.filename;
                si.found = bfd_find_inliner_info(abfd,
                                                 &filename,
                                                 &si.functionname,
                                                 &si.line);
                si.filename = (char *)filename;
            }
